...
class InvitationAuthenticator {
    encodeInvitation(invitation, secret) {
        return jwt.sign(
            invitation,
            secret,
            {expiresIn: config.invitation.expire},
        );
    }

    decodeInvitation(encodedInvitation, secret) {
        const verified = this._decodeOrNull(encodedInvitation, secret);
    ...
    _decodeOrNull(encodedInvitation, secret) {
        try {
            return jwt.verify(encodedInvitation, secret);
        } catch (e) {
            return null;
        }
    }
